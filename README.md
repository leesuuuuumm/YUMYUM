...
- 프론트엔드 PL(Proejct Leader)
  - 프론트엔드 파트를 리드하고 백엔드와의 협업을 위한 소통을 담당
  - 프론트엔드 파트의 업무 조율
...
- 개인프로젝트이지만 팀으로 대응!! (이슈 발생 시 팀원 공유)
- 팀프로젝트 개발 환경 구축
...
- 같은 파트 내의 사람들과 원활한 협업과 소통
- 각 파트별로 사용하는 프레임워크에 대한 숙련도 상승
...
1. 회원 가입 페이지
   - 이메일, 이름, 비밀번호로 가입 가능
   - 가입 필수 항목 모두 입력 시에만 '가입완료' 버튼 활성화
   - 이메일, 비밀번호 등 입력 기준이 충족되지 못할 시 오류 메시지 노출
2. 로그인 페이지
   - 이메일, 비밀번호 입력 기준이 충족되지 못할 시 오류 메시지 노출
   - 로그인 실패 시 사용자에게 실패 사유 에러메시지 노출
3. 계정 설정
   - 계정 설정에서 입력된 데이터가 변경되지 않았을 경우 '저장' 버튼 비활성화
...
1. 휴대폰으로 찍은 영상을 화면에 보여주고 서버에 저장
2. 지도 API를 활용해서 정확한 가게 위치 정보를 입력
3. 먹은 음식을 제목으로 지정하고 별점과 내용을 입력

#### 피드 페이지

1. 피드 페이지의 배경으로 영상을 자동 반복 재생
2. 피드 페이지에서 피드 수정, 공유 기능

#### 피드 플립 페이지

1. 다른 유저가 작성한 피드를 접할 수 있는 페이지
2. 좋아요를 눌러 마이페이지에서 확인 가능
...
1. 지도에 내가 작성한 피드, 다른 유저가 작성한 피드, 내가 좋아요한 피드를 구별해서 마커 표시
2. 마커를 누르면 해당 맛집으로 작성된 피드 목록 조회
